...
bool Gui::keyCallback(igl::opengl::glfw::Viewer &viewer, unsigned int key,
	...
	case ',':
		...
		break;
	case 'f':
		GuiData::instance->m_selectedField = GuiData::fluidField;
		return true;
	case 'F':
	case 'v':
		GuiData::instance->m_brushMode = GuiData::fluidBrush;
		return true;
	case 'g':
		GuiData::instance->m_selectedField = GuiData::pressureField;
		return true;
	case 'G':
	case 'h':
		GuiData::instance->m_selectedField = GuiData::tempField;
		return true;
	case 'H':
	case 'n':
		GuiData::instance->m_brushMode = GuiData::temperatureBrush;
		return true;
	case '1':
		GuiData::instance->m_brushTexture = GuiData::rectBrushTex;
		return true;
	case '2':
		GuiData::instance->m_brushTexture = GuiData::arrowBrushTex;
		return true;
	case '3':
		GuiData::instance->m_brushTexture = GuiData::bunnyBrushTex;
		return true;
	default:
...
bool Gui::mouseCallback(igl::opengl::glfw::Viewer &viewer,
	...
	if (minDist < 20) {
		...
		if (button == GLFW_MOUSE_BUTTON_LEFT)
			p_simulator->p_simulation->OnMouseClicked(m_viewer.data_list[m_clickedObject].V.row(vertex), modifier & GLFW_MOD_CONTROL);
		showVertexArrow();
	}
	return false;
}
...
void Gui::drawMenuWindow(igl::opengl::glfw::imgui::ImGuiMenu &menu) {
	...
	if (m_showStats) {
		...
		ImGui::Text("FPS: %.2f", 1e3/m_timerAverage);
		ImGui::Text("Particles: %d", p_simulator->p_simulation->getParticleCount());
        ImGui::Text("Fluid Cells: %d", p_simulator->p_simulation->getFluidCellCount());
		drawSimulationStats();
		ImGui::PopItemWidth();
		ImGui::End();
	}
